...
@Component
public class GetHierarchyLength?(name)DirectoriesFeature extends Feature {

    private final CommonApplicationProperties properties;
    private final ?(name)Service service;

    @Autowired
    public GetHierarchyLength?(name)DirectoriesFeature(CommonApplicationProperties properties, ?(name)Service service) {
    ...
    private void init() throws ClassNotFoundException {
        BOSObject bOSObject = new BOSObject("?(id)", null, "java.lang.Long", "?(name)", "?(synonym)", "Отримати поточну глибину ієрархії довідника '?(name)'");

        name = "Отримати поточну глибину ієрархії довідника '?(name)'";
        description = "Функціональність, яка надає можливість отримувати глибину ієрархії усіх довідників '?(name)'";
