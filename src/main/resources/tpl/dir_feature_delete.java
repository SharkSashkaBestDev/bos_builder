...
@Component
public class Delete?(name)DirectoriesFeature extends Feature {

    private final CommonApplicationProperties properties;
    private final ?(name)Service service;

    @Autowired
    public Delete?(name)DirectoriesFeature(CommonApplicationProperties properties, ?(name)Service service) {
    ...
    private void init() throws ClassNotFoundException {
        BOSObject bOSObject = new BOSObject("?(id)", null, "java.lang.Boolean", "?(objectPresentationCamelcase)", "?(objectPresentation)", "Видалити існуючі відомості про довідник '?(name)'");

        name = "Видалити існуючі відомості про довідник '?(name)'";
        description = "Функціональність, яка надає можливість видаляти існуючі відомості про довідник '?(name)'";
