...
@Component
public class FindAll?(name)DocumentsFeature extends Feature {

    private final CommonApplicationProperties properties;
    private final ?(name)Service service;

    @Autowired
    public FindAll?(name)DocumentsFeature(CommonApplicationProperties properties, ?(name)Service service) {
    ...
    private void init() throws ClassNotFoundException {
        BOSObject bOSObject = new BOSObject("?(id)", null, "java.util.List", "?(name)", "?(synonym)", "Відомості про усі документи '?(name)'");

        name = "Відомості про усі документи '?(name)'";
        description = "Функціональність, яка надає можливість отримувати список усіх документів '?(name)'";
